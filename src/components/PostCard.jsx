...
const PostCard = ({ post, user, deletePost, likePost, onClick }) => { 
  ...
  return (
    <div className='mb-2 bg-primary p-4 rounded-xl border hover:bg-gray-600'>
      {/* Remove the Link wrapper from here */}
      <div onClick={onClick} className="block cursor-pointer"> {/* Use onClick prop */}
        ...
        <div>
          <h1 className="text-white text-xl font-bold mb-2">I Built a 2D Game in 40 Minutes with Ebiten</h1>
          {/* <p className="text-white text-m font-semi mb-2">
          {showAll === post?._id
              ? post?.description
              : post?.description.slice(0, 150)}

            {post?.description?.length > 151 &&
              (showAll === post?._id ? 
              (
                <span
          ...
          <p className="text-white text-m font-semi mb-2">#Hashtags</p>
          {post?.image && (
            <img
              src={post?.image}
              alt='post image'
